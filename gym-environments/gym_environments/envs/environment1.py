...
class Env1(gym.Env):
    ...
    def __init__(self):
        ...
        # 记录每条边的相邻边，将mainEdge的id存在first中，将它的一条邻边存在second中，并且first[x]与second[x]对应
        self.first = None
        ...
        self.max_demand = 0
        # 对于每一个src,dst piar限制可选的paths为K个
        self.K = 4
        self.listofDemands = None
        self.nodes = None
        # 按照数字大小，将edge排序，包括edge中节点对的顺序
        self.ordered_edges = None
        # 为每条edge分配一个唯一的id，并记录到edgesDict
        self.edgesDict = None
        self.numNodes = None
        self.numEdges = None
        ...
        self.reward = 0
        # 储存所有节点两两之间的paths，并限制每个src,dst pair的paths数量为K
        self.allPaths = dict()
    ...
    def _first_second_between(self):
        ...
        for i, j in self.ordered_edges:
            ...
            for m, n in neighbour_edges:
                # 只要(i,j)和(m,n)不是同一条edge，就计算一次
                # self.first中是mainEdge, self.second中是neighbours，且main和neigh一一对应
    ...
    def make_step(self, state, action, demand, source, destination):
        ...
        # bw_allocated用到哪里了？在外面用到了(self.graph_state = np.copy(state))？
